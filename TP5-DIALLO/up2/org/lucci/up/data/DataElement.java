...
import java.util.*;
import org.lucci.up.data.event.DataElementListener;
import org.lucci.up.data.rendering.*;
import org.lucci.up.system.*;


/**
 * <p>A <code>DataElement</code> is the central notion of the system:
 * this is what will be graphically rendered.</p>
 * 
 * <p>A <code>DataElement</code> can be a single point or a figure
 * (a list of points and subfigures).</p>
 * 
 * <p>The rendering of this data element is made by its renderers.
 * A <code>DataElement</code> defines a list of renderers that will,
 * sequencely, render it on a graphical pane.</p>
 * 
 * @author Luc Hogie
 */
...
public abstract class DataElement implements Cloneable
{
	...
	/**
	 * Removes the given renderer. If the renderer is not found,
	 * nothing happens.
	 * @param renderer
	 */
	...
	/**
	 * Draw the data element on the given space.
	 * The process is trivial, sequencely, each
	 * renderer is asked to draw the data element
	 * on the space. That's all! The draw process
	 * is so delegated to each renderer.
	 * @param space
	 */
	public void draw( Space space )
	{
		...
		{
			DataElementRenderer renderer = (DataElementRenderer) rendererList.get( i );
